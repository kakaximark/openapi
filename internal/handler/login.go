...
func LoginHandler(c *gin.Context) {
	...
	if err != nil {
		...
		case service.ErrTooManyAttempts:
			attempts, lockoutRemaining, _ := service.GetRemainingAttempts(req.Username)
			c.JSON(statusCode, gin.H{
				...
				"data": gin.H{
					...
					"lockout_remaining":  lockoutRemaining.Minutes(),
				},
			})
			return
		case service.ErrInvalidPassword:
			attempts, _, _ := service.GetRemainingAttempts(req.Username)
			c.JSON(statusCode, gin.H{
				"code":    401,
				"message": message,
				"error":   err.Error(),
