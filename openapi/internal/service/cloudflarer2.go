...
func GetBucketInfo(countryCode, env, bucketName string) (*constants.CFResponse[constants.KVKeys], error) {
	...
	// 过滤数据，只保留包含 "latest" 或以 "v" 开头的数据
	var filteredResult []constants.KVKeys
	seenNames := make(map[string]bool) // 用于去重

	for _, item := range listObjectsOutput.CommonPrefixes {
		name := strings.TrimSuffix(*item.Prefix, "/") // 获取路径的第一部分作为名称
		logger.Info("Found matching item - Name: %s", name)
		// 检查是否已经添加过这个名称
		if seenNames[name] {
			continue
		}

		// 检查是否符合条件（包含 "latest" 或以 "v" 开头）
		if name == "latest" || strings.HasPrefix(name, "v") {
			filteredResult = append(filteredResult, constants.KVKeys{Name: name})
			seenNames[name] = true
			logger.Info("Found matching item - Name: %s", name)
		}
	}
	...
	logger.Info("Filtered bucket info - Total items: %d", len(filteredResult))

	// 打印调试信息

	logger.Info("Found %d directories and root files in bucket %s",
		len(listObjectsOutput.CommonPrefixes), bucketName)
